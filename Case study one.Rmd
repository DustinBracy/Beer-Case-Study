...
-Impute missing ABV and IBU values into BrewComplete (so we can compare vs Brewdata)
-Remove missing styles, 3 of these also have missing ABV and IBU values also. The ABV/IBU of the non-missings values aren't remarkable, so their removal shouldn't affect the overall data, but it's worth noting.
...
BrewComplete <- BrewComplete %>% filter(!is.na(Style))
...
```{r}
summary(BrewComplete$ABV)
sd(BrewComplete$ABV)
hist(BrewComplete$ABV)

# note the slight variation of the mean in BrewComplete vs original Brewdata
...
Several of the highly bitter beers have considerablly more alcohol in them than thier less bitter counterparts.
```{r}
BrewComplete %>% ggplot(aes(ABV, IBU)) + geom_jitter()
